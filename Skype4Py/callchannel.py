...
class ICallChannelManager(EventHandlingBase):
    ...
    automatically create a data channel for voice calls based on the APP2APP protocol.
      ...
      2. Events.
         ...
         A CallChannelManager object will allow you to assign some event handlers to events
         generated by Skype.
         ...
         This is done in three different ways which can be used simultaneously.
           ...
           1. Events object.
              ...
              You define your class resembling the L{ICallChannelManagerEvents} interface. You don't have to provide
              all event handlers, just the one you need. Then you pass an instance of this class as the
              I{Events} argument to the CallChannelManager object constructor (see L{ICallChannelManager.__init__}). You don't
              need to derive your class from L{ICallChannelManagerEvents}. It's just the methods names that matter.

           2. OnX properties.

              Write your event handlers as functions and assign them to OnX properties (where "X" is the
              name of the event) of CallChannelManager object. For the prototypes of the handlers, look at the
              L{ICallChannelManagerEvents} class. Note that your callables won't have the I{self} argument if they
           ...
           3. RegisterEventHandler / UnregisterEventHandler.
              ...
              CallChannelManager object features two methods that let you assign any number of callables to
              any event. Those are L{RegisterEventHandler} and L{UnregisterEventHandler}.
              For the names of events and prototypes of the handlers look at the L{ICallChannelManagerEvents} class.

    @ivar OnChannels: Event handler; a callable with L{ICallChannelManagerEvents.Channels} prototype (no 'self' argument).
    @type OnChannels: callable

    @ivar OnMessage: Event handler; a callable with L{ICallChannelManagerEvents.Message} prototype (no 'self' argument).
    @type OnMessage: callable

    @ivar OnCreated: Event handler; a callable with L{ICallChannelManagerEvents.Created} prototype (no 'self' argument).
    ...
    def __init__(self, Events=None):
        ...
        @param Events: Events
        @type Events: An optional object with event handlers. See L{ISkypeEvents} for possible event handlers and their
